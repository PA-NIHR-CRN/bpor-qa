...
test("search button", async ({ page }) => {
    ...
    searchButton.click();
    await page.waitForLoadState('networkidle');
    expect(page.url()).toEqual(process.env.URL+"results/search-results?query=&location=");
});
...
test("view conditoions button", async ({ page }) => {
    ...
    const viewConditionsButton = page.getByRole('link',{name: /View conditions/});
    await expect(viewConditionsButton).toHaveClass(/btn--nav/);
    viewConditionsButton.click();
    await page.waitForLoadState('networkidle');
    expect(page.url()).toEqual("https://bepartofresearch.nihr.ac.uk/results/a-z-conditions");
});
...
test("search term and location input", async ({ page }) => {
    ...
    await page.getByPlaceholder(/Keyword e.g. cancer, drug name/).fill("ISRCTN33951209");
    await page.locator('#location-search-field').fill("Strabane");
    page.getByRole('button',{name: /Search for study/}).click();
    await page.waitForLoadState('networkidle');
    await expect(page.url()).toEqual(process.env.URL+"results/search-results?query=ISRCTN33951209&location=Strabane");
});
