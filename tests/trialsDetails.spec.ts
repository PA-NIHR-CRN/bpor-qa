...
test("advert appears on page", async ({ page }) => {
    ...
    await page.goto(process.env.URL+"trial-details/trial-detail?trialId="+trialId+"&location=&distance=");
  ...
  test("JDR advert points to correct URL", async ({ page }) => {
    ...
    await expect(advertLink).toHaveAttribute('href','https://www.joindementiaresearch.nihr.ac.uk');
  });
  ...
  test("summary collapsable link", async ({ page }) => {
    ...
    await expect(collapsableContent).toHaveText(/This information is provided directly by researchers and we recognise that it isn't always easy to understand./);
  });